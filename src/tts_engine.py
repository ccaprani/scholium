...
class TTSEngine:
    ...
    def _create_silent_audio(self, output_path: str, duration: float, reference_audio_path: str = None):
        ...
        silent.export(str(output_path), format='mp3', bitrate='192k', parameters=["-ar", str(sample_rate)])


    def generate_segments(
        self, segments: List[Dict[str, Any]], voice_config: Dict[str, Any], output_dir: str,
        progress_callback=None
    ) -> List[Dict[str, Any]]:
        """Generate audio for multiple transcript segments.

        Args:
            segments: List of transcript segments from TranscriptParser (SlideSegment objects)
            voice_config: Voice configuration
            output_dir: Directory to save audio files

        Returns:
            List of segments with audio paths and durations:
            [
                {
                    'text': '...',
                    'slide_number': 1,
