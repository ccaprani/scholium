...
@click.option('--provider', required=True, type=click.Choice(['elevenlabs', 'coqui']), help='TTS provider')
...
def setup(name, provider, voice_id, description, config):
    ...
    if provider == 'elevenlabs':
        ...
        click.echo(f"   python src/main.py generate slides.md transcript.txt output.mp4 --voice {name}")
...
@cli.command()
@click.option('--name', required=True, help='Name for the voice')
@click.option('--provider', required=True, type=click.Choice(['coqui']), help='TTS provider (currently only coqui)')
@click.option('--sample', required=True, type=click.Path(exists=True), help='Audio sample for training')
@click.option('--description', help='Description of the voice')
